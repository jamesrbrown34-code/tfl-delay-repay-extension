...
export class ServiceDelayWorkflow {
  ...
  async handlePage() {
    ...
    switch (detectedStage) {
      case 'card-selection':
        this.statusPanel.update(this.domAdapter.getReadableWorkflowStage('card-selection'), `Submitted ${sdrAutofillState.completed?.length || 0}, remaining ${sdrAutofillState.queue?.length || 0}.`);
